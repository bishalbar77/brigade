...
| 10 | [10-ui-plan.md](10-ui-plan.md) | You're touching the guest UI or the ops tables — **built**, and it records the audit findings and the six places reality differed from the plan |

Also here: [image-credits.md](image-credits.md) — generated by `npm run images`. Attribution
for the 28 dish photographs, which CC BY-SA requires. The app shows the same data at
`/credits`.
